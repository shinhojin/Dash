...
installChaincode () {
	...
	ORG=$2
	setGlobals $PEER $ORG
	peer chaincode install -n supplycc -v 1.0 -l ${LANGUAGE} -p github.com/chaincode/foodcontract >&log.txt
	res=$?
	cat log.txt
...
instantiateChaincode () {
	...
	if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
		peer chaincode instantiate -o orderer.supply.com:7050 -C $CHANNEL_NAME -n supplycc -l ${LANGUAGE} -v 1.0 -c '{"Args":["init","order_001","John_1","100","5"]}' -P "OR	('Org1MSP.member','Org2MSP.member','Org3MSP.member')" >&log.txt
		res=$?
	else
		peer chaincode instantiate -o orderer.supply.com:7050 --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n supplycc -l ${LANGUAGE} -v 1.0 -c '{"Args":["init","order_001","John_1","100","5"]}' -P "OR	('Org1MSP.member','Org2MSP.member','Org3MSP.member')" >&log.txt
		res=$?
	fi
	cat log.txt
...
chaincodeQuery () {
  ...
  do
     ...
     peer chaincode query -C $CHANNEL_NAME -n supplycc -c '{"Args":["query","order_001"]}' >&log.txt
  done
  echo
  cat log.txt
}
...
chaincodeInvokeCreateRawFood() {
	...
	setGlobals $PEER $ORG
	if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
		peer chaincode invoke -o orderer.supply.com:7050 -C $CHANNEL_NAME -n supplycc -c '{"Args":["createRawFood","order_001"]}' >&log.txt
	else
		peer chaincode invoke -o orderer.supply.com:7050  --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n supplycc -c '{"Args":["createRawFood","order_001"]}' >&log.txt
	fi
	res=$?
	cat log.txt
...
chaincodeInvokeManufactureProcessing() {
	...
	setGlobals $PEER $ORG
	if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
		peer chaincode invoke -o orderer.supply.com:7050 -C $CHANNEL_NAME -n supplycc -c '{"Args":["manufactureProcessing","order_001"]}' >&log.txt
	else
		peer chaincode invoke -o orderer.supply.com:7050  --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n supplycc -c '{"Args":["manufactureProcessing","order_001"]}' >&log.txt
	fi
	res=$?
	cat log.txt
...
chaincodeInvokeWholesalerDistribute () {
	...
	setGlobals $PEER $ORG
	if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
		peer chaincode invoke -o orderer.supply.com:7050 -C $CHANNEL_NAME -n supplycc -c '{"Args":["wholesalerDistribute","order_001"]}' >&log.txt
	else
		peer chaincode invoke -o orderer.supply.com:7050  --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n supplycc -c '{"Args":["wholesalerDistribute","order_001"]}' >&log.txt
	fi
	res=$?
	cat log.txt
...
chaincodeInvokeInitiateShipment () {
	...
	setGlobals $PEER $ORG
	if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
		peer chaincode invoke -o orderer.supply.com:7050 -C $CHANNEL_NAME -n supplycc -c '{"Args":["initiateShipment","order_001"]}' >&log.txt
	else
		peer chaincode invoke -o orderer.supply.com:7050  --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n supplycc -c '{"Args":["initiateShipment","order_001"]}' >&log.txt
	fi
	res=$?
	cat log.txt
...
chaincodeInvokeDeliverToRetailer () {
	...
	setGlobals $PEER $ORG
	if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
		peer chaincode invoke -o orderer.supply.com:7050 -C $CHANNEL_NAME -n supplycc -c '{"Args":["deliverToRetail","order_001"]}' >&log.txt
	else
		peer chaincode invoke -o orderer.supply.com:7050  --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n supplycc -c '{"Args":["deliverToRetail","order_001"]}' >&log.txt
	fi
	res=$?
	cat log.txt
...
chaincodeInvokeCompleteOrder () {
	...
	setGlobals $PEER $ORG
	if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
		peer chaincode invoke -o orderer.supply.com:7050 -C $CHANNEL_NAME -n supplycc -c '{"Args":["completeOrder","order_001"]}' >&log.txt
	else
		peer chaincode invoke -o orderer.supply.com:7050  --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n supplycc -c '{"Args":["completeOrder","order_001"]}' >&log.txt
	fi
	res=$?
	cat log.txt
